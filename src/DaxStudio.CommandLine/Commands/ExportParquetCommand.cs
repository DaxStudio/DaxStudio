...
{
    internal class ExportParquetCommand : AsyncCommand<ExportParquetCommand.Settings>
    {
        internal class Settings: CommandSettingsFolderBase
        {

            //[CommandArgument(0,"[folder]")]
            //[Description("The folder where the parquet files will be written")]
        ...
        public ExportParquetCommand(IEventAggregator eventAggregator, IGlobalOptions options)
        {
            ...
            await AnsiConsole.Progress()
                ...
                .StartAsync(async ctx =>
                {
                    try
                    {
                        ...
                        Log.Information($"Exporting {SelectedTables.Count} table{(SelectedTables.Count > 1 ? "s" : "")} to Parquet files");
                        await vm.ExportDataToParquetFilesAsync(settings.OutputFolder, SelectedTables);
                        connMgr.Close();
