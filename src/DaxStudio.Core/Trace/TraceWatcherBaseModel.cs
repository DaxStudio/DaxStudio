...
namespace DaxStudio.Core.Trace
{
        ...
        //, IHandle<TraceChangingEvent>
    {
        ...
        protected TraceWatcherBaseModel(IEventAggregator eventAggregator, IGlobalOptions globalOptions, IWindowManager windowManager)
        {
        ...
        protected bool CanHideTrace(object obj)
        {
            return CanHide;
        }
            ...
            set
            {
                ...
                if (_isChecked != value)
                {
                    ...
                    if (value)
                    {
                        ...
                        {
                            Log.Error(ex, Constants.LogMessageTemplate, GetSubclassName(), nameof(IsChecked), "error setting IsChecked");
        ...
        private async Task StartTraceAsync()
        {
            await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    Log.Verbose(Constants.LogMessageTemplate, GetSubclassName(), nameof(StartTraceAsync),
        ...
        public virtual bool CanExport { get { return true; } }

        public void Export()
        {
            var filePath = PromptExportFilePath();
            if (string.IsNullOrEmpty(filePath)) return;

            try
            {
                ExportTraceDetails(filePath);
            }
            catch (Exception ex)
            {
        ...
        // UI shells override this to prompt the user (e.g. with a SaveFileDialog)
        // and return the selected file path, or null/empty to cancel.
        protected virtual string PromptExportFilePath()
        {
            return null;
        }
        ...
        public void CreateTracer()
        {
            try
            {
                if (!Connection.IsConnected)
                {
                    ...
                    Log.Error(Constants.LogMessageTemplate, GetSubclassName(), nameof(CreateTracer), msg);
                    return;
                }
        ...
        public async Task StopTraceAsync()
        {
            await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    Log.Verbose(Constants.LogMessageTemplate, GetSubclassName(), nameof(StopTraceAsync),
                        "Stopping Tracer");
                    await _eventAggregator.PublishAsync(new TraceChangedEvent(this, QueryTraceStatus.Stopped));
                }
                catch (Exception ex)
