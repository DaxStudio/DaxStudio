...
#if NETFRAMEWORK
                var tomToken = _connectionManager.AccessToken.ToTomAccessToken();
#else
                var tomToken = _connectionManager.AccessToken; // Same type on .NET 8
#endif
                if (tomToken.IsNotNull())
                {
                    _server.AccessToken = tomToken;
