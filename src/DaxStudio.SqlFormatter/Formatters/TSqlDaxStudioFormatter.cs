 ...
 * Timothy Klenke, 2012
 * Marco Russo, 2023 (implementation for DAX Studio)

This class is an implementation based on TSqlStandardFormatterOptions 
modified to support the formatting of storage engine queries in DAX Studio.
The lack of virtual methods in the original implementation does not allow
a simple inheritance for customization.
...
{
    public class TSqlDaxStudioFormatter : ISqlTreeFormatter
    {
        public TSqlDaxStudioFormatter() : this(new TSqlStandardFormatterOptions()) { }

        public TSqlDaxStudioFormatter(TSqlStandardFormatterOptions options)
        {
            if (options == null)
        ...
        private void ProcessSqlNode(Node contentElement, TSqlStandardFormattingState state)
        {
            ...
            switch (contentElement.Name)
            {
                ...
                case SqlStructureConstants.ENAME_SET_OPERATOR_CLAUSE:
                    // Changes for SQL code generated by DirectQuery: UNION ALL is often used to combine static keys,
                ...
                case SqlStructureConstants.ENAME_IN_PARENS:
                    // Replace internal query with table name (assuming there are no M transformations)
                    if (contentElement.Children.Any(n1 => n1.Children.Any(n2 => n2.TextValue == "$Table")))
                    {
                        // Find children branch with FROM condition
                        var fromLevel = contentElement.Children.LastOrDefault(n1 => n1.Children.Any(n2 => n2.TextValue.ToUpperInvariant() == "FROM"));
                        if (fromLevel != null)
                        {
                            var fromKeyword = fromLevel.Children.LastOrDefault(n => n.TextValue.ToUpperInvariant() == "FROM");
                            if (fromKeyword != null)
                            {
                                var fromCondition = fromLevel.Children.SkipWhile(n => n != fromKeyword).Skip(1).FirstOrDefault();
                                var replaceTable = fromCondition.Children.TakeWhile(n => n.TextValue.ToUpperInvariant() != "AS");
                                if (replaceTable?.Any() == true)
                                {
                                    WhiteSpace_SeparateStatements(contentElement, state);
                                    state.ResetKeywords();
                                    ProcessSqlNodeList(replaceTable, state);
