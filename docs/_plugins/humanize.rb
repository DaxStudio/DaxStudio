  ...
  module Humanize
    ...
    def filesize(value, round=3)
      ...
      if value >= filesize_tb
        return "%s TB" % (value / filesize_tb).to_f.round(round)
      elsif value >= filesize_gb
        return "%s GB" % (value / filesize_gb).to_f.round(round)
      elsif value >= filesize_mb
        return "%s MB" % (value / filesize_mb).to_f.round(round)
